...
fn counts_as_useful_match<I>(scratch: &SearchScratch<I>, neighbor: Neighbor<I>) -> bool
where
    I: VectorId,
{
    scratch.best.size() < scratch.best.capacity()
        || match scratch.best.iter().last() {
            Some(worst) => neighbor.distance < worst.distance,
            None => true,
        }
}
    ...
    SR: SearchRecord<I> + ?Sized,
{
    ...
    while scratch.best.has_notvisited_node() && !accessor.terminate_early() {
        ...
        let mut useful_hop_matches = 0usize;
        ...
        for neighbor in one_hop_neighbors.iter().copied() {
            match query_label_evaluator.on_visit(neighbor) {
                QueryVisitDecision::Accept(accepted) => {
                    let counts_toward_stop = counts_as_useful_match(scratch, accepted);
                    scratch.best.insert(accepted);
                    hop_match_count += 1;
                    if counts_toward_stop {
                        useful_hop_matches += 1;
                    }
                }
        ...
        if useful_hop_matches * 4 >= one_hop_neighbors.len() {
        ...
        let useful_hop_match_limit = max_degree_with_slack / 4;

        for candidate in candidates_two_hop_expansion.iter().copied() {
            if useful_hop_matches >= useful_hop_match_limit || accessor.terminate_early() {
                break;
            }
            ...
            for neighbor in two_hop_neighbors.iter().copied() {
                let counts_toward_stop = counts_as_useful_match(scratch, neighbor);
